...
Although multimodal GFMs such as <<msGFM, msGFM>> exist and incorporate LiDAR as one of the modalities, its use is limited to 2.5D representations such as DSM, and has not yet extended to handling full 3D information with point clouds as direct input. +
...
|<<PointMamba, PointMamba>> |Mamba-based | point-based |Supervised training |3D representation learning model that efficiently captures long-range dependencies using the Mamba state space model. It applies linear-complexity sequence modeling—positioned as an alternative to Transformers to point clouds.
|<<Point-DAE, Point-DAE>> |Backbone-agnostic | point-based |DAE |A generalized denoising autoencoder framework for point cloud representation learning that reconstructs clean point clouds from corrupted inputs, leveraging both local (masking) and global (affine transformation) corruptions to enhance geometric understanding.
|===
...
|<<ShapeNet, ShapeNet>> |Semantic segmentation |Point cloud |The dataset itself is primarily treated as a benchmark.
|<<Building-PCC, Building-PCC>> |Point cloud completion |Point cloud |Building Point Cloud Completion Benchmarks
|===
...
|<<Semantic3D, Semantic3D>> |Semantic Segmentation, Object Detection |Point cloud(outdoor) | Large-Scale Point Cloud Classification Benchmark
|===
...
|<<KITTI, KITTI>> |Semantic Segmentation, Panoptic Segmentation, 4D Panoptic Segmentation, Moving Object Segmentation, Semantic Scene Completion |Point cloud(outdoor) |<<SemanticKITTI, SemanticKITTI>>
|<<OpenGF, OpenGF>> |Semantic Segmentation, Terrain scene recognition |Point cloud(outdoor) |The dataset itself is primarily treated as a benchmark.
|===
...
Although several largescale datasets, such as <<OpenGF, OpenGF>> and <<PureForest, PureForest>>, exist, they lack the scale and land cover diversity necessary for training versatile models. Furthermore, while numerous freely available ALS LiDAR data sources, such as the United States Geological Survey 3D Elevation Program (USGS 3DEP) and the Actueel Hoogtebestand Nederland (AHN), provide extensive resources, there is currently a few efficient method to extract data from these sources. Leveraging all available data is computationally prohibitive and often redundant. These limitations collectively hinder progress in adopting the pre-training and fine-tuning paradigm for GFM. +
//
With the development of methods that extract diverse data from sources such as <<ALS-pretraining, ALS-pretraining>>, there is a growing need for the construction of large-scale datasets for pre-training.
...
Therefore, if a core set of Geospatial tasks were established, GFM developers would be able to determine which subset of tasks their resulting GFM can effectively adapt to.
