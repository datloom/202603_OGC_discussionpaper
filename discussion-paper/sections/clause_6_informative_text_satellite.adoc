...
|<<Prithvi-EO-2.0>> |transformer-based |MAE |Landsat 8, 9 and Sentinel-2 |A multi-temporal model trained on global time series data that incorporates temporal and location embeddings, with 600M parameters which is among the largest in the field of EO.
|<<DOFA>> |transformer-based |MIM |Sentinel-1, Sentinel-2, Gaofen-2, NAIP, EnMAP |A multi-modal model leverages wavelength as a unifying parameter across various EO modalities.
|===
